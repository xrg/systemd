...
int exec_spawn(ExecCommand *command,
        ...
        if (pid == 0) {
                ...
                err = setup_output(context, STDOUT_FILENO, socket_fd, path_get_file_name(command->path), unit_id, apply_tty_stdin, uid, gid);
                if (err < 0) {
                        r = EXIT_STDOUT;
                        goto fail_child;
                }

                err = setup_output(context, STDERR_FILENO, socket_fd, path_get_file_name(command->path), unit_id, apply_tty_stdin, uid, gid);
                if (err < 0) {
                        r = EXIT_STDERR;
                        goto fail_child;
                }
